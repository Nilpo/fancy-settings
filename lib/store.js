...
(function () {
		...
		this.storage.get(null, function(items) {
			...
			for(var key in items) {
				if(key.substring(0, name.length) === name) {
					...
					var value = items[key];
					if(value == "store." + self.name + "." + name + "___CHECK_LOCAL___") {
						value = localStorage.getItem("store." + self.name + "_localonly." + name);
					}
					self.stateSet(thisKey, value);
				}
			}
			self.inSync = true;
    ...
    Store.prototype.get = function (key, callback) {
		if(this.inSync) {
			...
			this.storage.get(fullKey, function(items) {
				...
				if(value == "store." + self.name + "." + key + "___CHECK_LOCAL___") {
					value = localStorage.getItem("store." + this.name + "_localonly." + key);
				}
    ...
    Store.prototype.set = function (key, value, localOnly) {
        if (value === undefined) {
            this.remove(key);
        } else {
			...
			if(localOnly !== true) {
				obj["store." + this.name + "." + key] = value;
			} else {
				obj["store." + this.name + "." + key] = "store." + this.name + "." + key + "___CHECK_LOCAL___";
				localStorage.setItem("store." + this.name + "_localonly." + key, value);
			}
